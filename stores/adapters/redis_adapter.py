...
def get_stores():
    ...
    return redis.Redis(host=HOST, db=STOREDB).hkeys("stores")
...
def create_store(store_name, initial_data=[]):
    ...
    stores_db = edis.Redis(host=HOST, db=STOREDB)
    stores_db.hset(store_name, stores_db.get("next"))
    stores_db.incr("next")
    return {"store_name": store_name, "db_id": stores_db.hget("stores", store_name)}
...
def store_get_val(store_name, key):
